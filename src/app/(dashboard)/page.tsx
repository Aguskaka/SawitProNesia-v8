...
export default async function HomePage(){
 ...
 return <div className="commandCenter">
  <section className="commandHeading"><div><span>OWNER COMMAND CENTER</span><h1>Kondisi {estate.name}</h1><p>Ringkasan produksi, biaya, rencana, anggaran, dan aktivitas dalam satu layar.</p></div><ContextSelector estates={estates.map(e=>({id:e.id,name:e.name}))} selectedYear={context.selectedYear} activeEstateId={estate.id}/></section>
  <section className="commandKpis">
   <article><small>Produksi Tahun Ini</small><strong>{formatNumber(summary.productionKg)} Kg</strong><span>{productivity.toLocaleString("id-ID",{maximumFractionDigits:2})} ton/Ha</span></article>
   ...
   <article><small>Margin</small><strong>{formatCompactRupiah(summary.margin)}</strong><span>{marginPct.toLocaleString("id-ID",{maximumFractionDigits:1})}%</span></article>
   <article><small>Budget</small><strong>{formatCompactRupiah(annualBudget)}</strong><span>{usage.percentage.toLocaleString("id-ID",{maximumFractionDigits:1})}% terpakai</span></article>
  </section>
