...
export default async function BlockDetail({ params, searchParams }: { params: Promise<{ estateId: string; blockId: string }>; searchParams: Promise<{ status?: string }> }) {
  ...
  return <div className="masterPage"><Link href={`/kebun/${estateId}`} className="backLink">← {estate.name}</Link>{query.status === "updated" ? <div className="activityNotice">Perubahan blok berhasil disimpan.</div> : null}
    ...
    <section className="dangerPanel"><div><b>Hapus blok</b><p>Hanya dapat dilakukan bila tidak ditolak oleh relasi/constraint data transaksi di Supabase.</p></div><form action={deleteAction}><button className="dangerButton" type="submit">Hapus Blok</button></form></section>
  </div>;
}
