...
export default async function BudgetPage({searchParams}:{searchParams:Promise<{status?:string}>}) {
 ...
 return <div className="budgetPage">
  ...
  <section className="budgetGrid">
   <article className="budgetPanel"><div className="activitySectionTitle"><span>MASTER BUDGET</span><h2>Anggaran Tahunan</h2></div>{estate?<form action={saveAnnualBudget} className="budgetForm"><input type="hidden" name="estate_id" value={estate.id}/><input type="hidden" name="budget_year" value={context.selectedYear}/><label>Nilai Anggaran (Rp)<input name="amount" type="number" min="0" step="1" defaultValue={budget}/></label><button className="primaryButton">Simpan Anggaran</button></form>:null}</article>
   <article className="budgetPanel"><div className="activitySectionTitle"><span>KATEGORI</span><h2>Budget per Kategori</h2></div>{estate?<form action={saveCategoryBudget} className="budgetForm"><input type="hidden" name="estate_id" value={estate.id}/><input type="hidden" name="budget_year" value={context.selectedYear}/><label>Kategori<select name="category">{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select></label><label>Anggaran (Rp)<input name="amount" type="number" min="0" step="1"/></label><button className="primaryButton">Simpan Kategori</button></form>:null}</article>
   <article className="budgetPanel"><div className="activitySectionTitle"><span>PER BLOK</span><h2>Budget Blok</h2></div>{estate?<form action={saveBlockBudget} className="budgetForm"><input type="hidden" name="estate_id" value={estate.id}/><input type="hidden" name="budget_year" value={context.selectedYear}/><label>Blok<select name="block_id">{estateBlocks.map(b=><option value={b.id} key={b.id}>{b.name}</option>)}</select></label><label>Kategori<select name="category">{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select></label><label>Anggaran (Rp)<input name="amount" type="number" min="0" step="1"/></label><button className="primaryButton">Simpan Budget Blok</button></form>:null}</article>
  </section>
