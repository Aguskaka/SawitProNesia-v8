...
export function DashboardShell({ children, role }: { children: ReactNode; role: string }) {
  ...
  return <div className={`appShell premiumShell ${pemanen ? "pemanenShell" : ""}`}>
    ...
    <aside className="sideRail">
      ...
      <div className="sideFoot"><small>{pemanen ? "FIELD HARVEST MODE" : "SAWIT OPERATIONS OS"}</small><b>v10.7</b>{!pemanen ? <PwaInstallButton /> : null}<form action={logout}><button className="sideLogout" type="submit">Keluar</button></form></div>
    </aside>
    <div className="shellBody">
      <header className="mobileTopbar">
        <Link className="mobileBrand" href={pemanen ? "/panen" : "/"}><BrandMark/><b>SawitProNesia</b></Link>
        <div>{!pemanen ? <PwaInstallButton compact /> : <span className="rolePill">PEMANEN</span>}<span className="versionPill">v10.7</span><form action={logout}><button className="mobileAccount" type="submit" title="Keluar" aria-label="Keluar dari SawitProNesia"><AppIcon name="user"/></button></form></div>
      </header>
