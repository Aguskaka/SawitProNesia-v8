...
export function DashboardShell({ children, role, email }: { children: ReactNode; role: string; email?: string | null }) {
  ...
  return <div className={`appShell premiumShell ${pemanen ? "pemanenShell" : ""}`}>
    ...
    <aside className="sideRail">
      ...
      <div className="sideFoot"><small>{pemanen ? "FIELD HARVEST MODE" : "SAWIT OPERATIONS OS"}</small><b>v11.2.0</b>{!pemanen ? <PwaInstallButton /> : null}<form action={logout}><button className="sideLogout" type="submit">Keluar</button></form></div>
    </aside>
    <div className="shellBody">
      <header className="mobileTopbar">
        <Link className="mobileBrand" href={pemanen ? "/panen" : "/"}><BrandMark/><b>SawitProNesia</b></Link>
        <div className="topbarActions">{!pemanen ? <PwaInstallButton compact /> : <span className="rolePill">PEMANEN</span>}<span className="versionPill">v11.2.0</span><details className="accountMenu"><summary className="mobileAccount" title="Profil" aria-label="Buka menu profil"><AppIcon name="user"/></summary><div className="accountMenuPanel"><div className="accountMenuHead"><span className="accountAvatar"><AppIcon name="user"/></span><div><b>{email || "Akun SawitProNesia"}</b><small>{roleLabel(role)}</small></div></div><div className="accountMenuMeta"><span>Role</span><b>{roleLabel(role)}</b></div>{role === "owner" ? <Link className="accountMenuLink" href="/akses"><AppIcon name="user"/><span><b>Akses Pengguna</b><small>Kelola role & kebun tugas</small></span></Link> : null}<div className="accountMenuVersion"><span>Versi aplikasi</span><b>v11.2.0</b></div><form action={logout}><button className="accountLogout" type="submit">Keluar</button></form></div></details></div>
      </header>
